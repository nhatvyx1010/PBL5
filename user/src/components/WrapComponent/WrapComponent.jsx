...
// const WrapComponent = ({ totalItems, itemsPerPage, onSelect }) => {
...
//       onSelect(itemIndex); // Truyền vị trí của button là null vì button đã bị loại bỏ
//     }
//   };
...
//   itemsPerPage: PropTypes.array.isRequired,
...
// const WrapComponent = ({ selectedTrainId, totalItems, onSelect, selectedItems }) => {
const WrapComponent = ({ selectedButtons, totalItems, itemsPerPage, onSelect }) => {
  const [buttonsState, setButtonsState] = useState([]);

  useEffect(() => {
    console.log("buttonsState: ", buttonsState);
    console.log("selectedButtons: ", selectedButtons);
    // Tạo danh sách trạng thái clicked của các button dựa trên selectedButtons
    const newButtonsState = Array.from({ length: totalItems }, (_, i) => selectedButtons.includes(i + 1));
    setButtonsState(newButtonsState);
  }, [selectedButtons, totalItems]);

  const handleClick = (itemIndex) => {
    const newButtonsState = [...buttonsState];
    newButtonsState[itemIndex - 1] = !newButtonsState[itemIndex - 1];
    setButtonsState(newButtonsState);
    onSelect(itemIndex);
  };
